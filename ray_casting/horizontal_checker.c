...
/*   Updated: 2024/01/25 14:51:49 by mkhairal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	horizontal_casting(t_global_conf *config, t_ray_info *ray_info)
{
	...
	if (ray_info->is_ray_facing_down)
		ray_info->y_inter = MINIMAP_SCALE;
	ray_info->x_inter = \
		config->player->x + ((y_inter - config->player->y) / tan(ray_angle));
	ray_info->y_step = MINIMAP_SCALE;
	if (ray_info->is_ray_facing_up)
		ray_info *= -1;
	ray_info->x_step = MINIMAP_SCALE / tan(ray_angle);
	if (ray_info->is_ray_facing_left && ray_info->x_step > 0)
		ray_info->x_step *= -1;
	else if (ray_info->is_ray_facing_right && ray_info->x_step < 0)
		ray_info->x_step *= -1;
